...
def parse_vid_rec(filename, classhash, img_ids, defaultIOUthr=0.5, pixelTolerance=10):
    ...
    for obj in tree.findall('object'):
        ...
        thr = (gt_w * gt_h) / ((gt_w + pixelTolerance)
                               * (gt_h + pixelTolerance))
...
def vid_eval(detpath, annopath, imageset_file, classname_map, annocache, ovthresh=0.5):
    ...
    if not os.path.isfile(annocache):
        ...
        for ind, image_filename in enumerate(img_basenames):
            recs.append(parse_vid_rec(annopath.format(
                'VID/' + image_filename), classhash, gt_img_ids[ind]))
            if ind % 100 == 0:
                print(
                    'reading annotations for {:d}/{:d}'.format(ind + 1, len(img_basenames)))
        ...
        for j in range(0, num_obj):
            ...
            for k in range(0, num_gt_obj):
                ...
                if gt_detected[k] > 0:
                    continue
                bbgt = gt_bboxes[k, :]
                bi = [np.max((bb[0], bbgt[0])), np.max((bb[1], bbgt[1])), np.min(
                    (bb[2], bbgt[2])), np.min((bb[3], bbgt[3]))]
                iw = bi[2] - bi[0] + 1
                ih = bi[3] - bi[1] + 1
                if iw > 0 and ih > 0:
    ...
    tp_all = np.concatenate(
        [x for x in np.array(tp_cell)[gt_img_ids] if x is not None])
    fp_all = np.concatenate(
        [x for x in np.array(fp_cell)[gt_img_ids] if x is not None])
    obj_labels = np.concatenate([x for x in np.array(
        obj_labels_cell)[gt_img_ids] if x is not None])
    confs = np.concatenate([x for x in np.array(
        obj_confs_cell)[gt_img_ids] if x is not None])
