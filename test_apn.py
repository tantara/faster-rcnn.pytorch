...
if __name__ == '__main__':
  ...
  def get_models(args, cfg):
      ...
      if args.net == 'vgg16':
        fasterRCNNKey = vgg16_key(imdb.classes, pretrained=False, class_agnostic=args.class_agnostic)
        fasterRCNNCur = vgg16_cur(imdb.classes, pretrained=False, class_agnostic=args.class_agnostic)
      elif args.net == 'res101':
        fasterRCNNKey = resnet_key(imdb.classes, 101, pretrained=False, class_agnostic=args.class_agnostic)
        fasterRCNNCur = resnet_cur(imdb.classes, 101, pretrained=False, class_agnostic=args.class_agnostic)
      elif args.net == 'res50':
        fasterRCNNKey = resnet_key(imdb.classes, 50, pretrained=False, class_agnostic=args.class_agnostic)
        fasterRCNNCur = resnet_cur(imdb.classes, 50, pretrained=False, class_agnostic=args.class_agnostic)
      elif args.net == 'res152':
        fasterRCNNKey = resnet_key(imdb.classes, 152, pretrained=False, class_agnostic=args.class_agnostic)
        fasterRCNNCur = resnet_cur(imdb.classes, 152, pretrained=False, class_agnostic=args.class_agnostic)
      else:
  ...
  # create predictor
  key_predictors = [get_predictor(models[i][0], dataloaders[i], cfg, imdb, gpu_id) for i, gpu_id in enumerate(gpus)]
  cur_predictors = [get_predictor(models[i][1], dataloaders[i], cfg, imdb, gpu_id) for i, gpu_id in enumerate(gpus)]
  ...
  def pred_eval_multiprocess(gpus, key_predictors, cur_predictors, test_datas, imdb, cfg, roidbs_seg_lens=roidbs_seg_lens, vis=False, thresh=1e-4, ignore_cache=True,):
    start = time.time()
    if len(gpus) == 1:
        res = [pred_eval(0, 0, key_predictors[0], cur_predictors[0], test_datas[0], roidbs[0], imdb, cfg, roidbs_seg_lens, vis, thresh, ignore_cache),]
    else:
        print('multiple processes')
        pool = Pool(processes=len(gpus))#, ctx=None)
        multiple_results = [pool.apply_async(pred_eval,args=(i, gpu_id, key_predictors[i], cur_predictors[i], test_datas[i], roidbs[i], imdb, cfg, roidbs_seg_lens, vis, thresh, ignore_cache)) for i, gpu_id in enumerate(gpus)]
        pool.close()
        pool.join()
  ...
  pred_eval_multiprocess(gpus, key_predictors, cur_predictors, dataloaders, imdb, cfg, roidbs_seg_lens=roidbs_seg_lens, vis=vis, ignore_cache=args.ignore_cache, thresh=thresh)
